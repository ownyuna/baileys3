# <div align='center'>baileys3 - Typescript/Javascript WhatsApp Web API</div>

<div align="center">

<img src="https://files.catbox.moe/w4ew7v.jpg" alt="Header Image" width="100%"/>

<br/>

<!-- Badges -->
<p>
  <img src="https://img.shields.io/npm/dw/%40itsukichan%2Fbaileys3?label=npm&color=%23CB3837" alt="NPM Downloads"/>
  <img src="https://img.shields.io/github/v/release/ownyuna/baileys3?include_prereleases&sort=semver" alt="Latest Release"/>
  <img src="https://img.shields.io/github/languages/code-size/ownyuna/baileys3" alt="Code Size"/>
  <img src="https://img.shields.io/github/license/ownyuna/baileys3" alt="License"/>
  <img src="https://img.shields.io/github/stars/ownyuna/baileys3" alt="Stars"/>
  <img src="https://img.shields.io/github/forks/ownyuna/baileys3" alt="Forks"/>
</p>

<!-- GitHub Stats -->
<p>
  <img src="https://github-readme-stats.vercel.app/api?username=ownyuna&show_icons=true&theme=radical" alt="GitHub Stats"/>
</p>
...
baileys3 and its maintainers cannot be held liable for misuse of this application, as stated in the [MIT license](https://github.com/WhiskeySockets/baileys3/blob/master/LICENSE).
The maintainers of baileys3 do not in any way condone the use of this application in practices that violate the Terms of Service of WhatsApp. The maintainers of this application call upon the personal responsibility of its users to use this application in a fair way, as it is intended to be used.
##

- baileys3 does not require Selenium or any other browser to be interface with WhatsApp Web, it does so directly using a **WebSocket**. 
- Not running Selenium or Chromimum saves you like **half a gig** of ram :/ 
- baileys3 supports interacting with the multi-device & web versions of WhatsApp.
...
```
yarn add @itsukichan/baileys3
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:ownyuna/baileys3
```

Then import your code using:
```ts 
import makeWASocket from '@itsukichan/baileys3'
```
...
- [Writing Custom Functionality](#writing-custom-functionality)
    - [Enabling Debug Level in baileys3 Logs](#enabling-debug-level-in-baileys3-logs)
    - [How Whatsapp Communicate With Us](#how-whatsapp-communicate-with-us)
    - [Register a Callback for Websocket Events](#register-a-callback-for-websocket-events)

## Connecting Account

WhatsApp provides a multi-device API that allows baileys3 to be authenticated as a second WhatsApp client by scanning a **QR code** or **Pairing Code** with WhatsApp on your phone.

> [!NOTE]
> **[Here](#example-to-start) is a simple example of event handling**

> [!TIP]
> **You can see all supported socket configs [here](https://baileys3.whiskeysockets.io/types/SocketConfig.html) (Recommended)**

### Starting socket with **QR-CODE**

> [!TIP]
> You can customize browser name if you connect with **QR-CODE**, with `Browser` constant, we have some browsers config, **see [here](https://baileys3.whiskeysockets.io/types/BrowsersMap.html)**
...
2. baileys3, by default, use chrome browser config
...
- If you use baileys3 for groups, we recommend you to set `cachedGroupMetadata` in socket config, you need to implement a cache like this:
...
```ts
import makeWASocket, { useMultiFileAuthState } from '@itsukichan/baileys3'

const { state, saveCreds } = await useMultiFileAuthState('auth_info_baileys3')
...
- baileys3 uses the EventEmitter syntax for events. 
They're all nicely typed up, so you shouldn't have any issues with an Intellisense editor like VS Code.

> [!IMPORTANT]
> **The events are [these](https://baileys3.whiskeysockets.io/types/baileys3EventMap.html)**, it's important you see all events
...
```ts
import makeWASocket, { DisconnectReason, useMultiFileAuthState } from '@itsukichan/baileys3'
...
```ts
import makeWASocket, { useSingleFileAuthState, useMongoFileAuthState } from '@itsukichan/baileys3'

// Single Auth
const { state, saveState } = await useSingleFileAuthState('./auth_info_baileys3.json') 
const suki = makeWASocket({
        auth: state,
        printQRInTerminal: true
    })
...
const connectAuth = async() => {
  ...
  const collection = client.db("@ownyuna").collection("sessions")
  return collection
}
...
import { makeInMemoryStore, getAggregateVotesInPollMessage } from '@itsukichan/baileys3'

const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: "@ownyuna" })
logger.level = "fatal"
...
- baileys3 does not come with a defacto storage for chats, contacts, or messages. However, a simple in-memory implementation has been provided. The store listens for chat updates, new messages, message updates, etc., to always have an up-to-date version of the data.
...
```ts
import makeWASocket, { makeInMemoryStore } from '@itsukichan/baileys3'
// the store maintains the data of the WA connection in memory
// can be written out to a file & read from it
const store = makeInMemoryStore({ })
// can be read from a file
store.readFromFile('./baileys3_store.json')
// saves the state to a file every 10s
setInterval(() => {
    store.writeToFile('./baileys3_store.json')
}, 10_000)
...
- Send all types of messages with a single function
    - **[Here](https://baileys3.whiskeysockets.io/types/AnyMessageContent.html) you can see all message contents supported, like text message**
    - **[Here](https://baileys3.whiskeysockets.io/types/MiscMessageGenerationOptions.html) you can see all options supported, like quote message**
...
#### Forward Messages
- You need to have message object, can be retrieved from [store](#implementing-a-data-store) or use a [message](https://baileys3.whiskeysockets.io/types/WAMessage.html) object
```ts
...
#### Reaction Message
- You need to pass the key of message, you can retrieve from [store](#implementing-a-data-store) or use a [key](https://baileys3.whiskeysockets.io/types/WAMessageKey.html) object
```ts
await suki.sendMessage(
    jid,
    {
       ...
       interactiveButtons: [ 
          { 
             ...
             buttonParamsJson: JSON.stringify({ 
                ...
                additional_note: 'ownyuna Kawaii >\\\<', 
...
2. baileys3 has a function to generate the content for these link previews
...
await suki.sendMessage(
    jid,
    {
        text: 'Hi, this was sent using https://github.com/whiskeysockets/baileys3'
    }
)
```
...
> [!NOTE]
> In media messages, you can pass `{ stream: Stream }` or `{ url: Url }` or `Buffer` directly, you can see more [here](https://baileys3.whiskeysockets.io/types/WAMediaUpload.html)

- When specifying a media url, baileys3 never loads the entire buffer into memory it even encrypts the media as a readable stream.
...
import { downloadMediaMessage, getContentType } from '@itsukichan/baileys3'
...
suki.ev.on('messages.upsert', async ({ [m] }) => {
    ...
    if (messageType === 'imageMessage') {
        ...
        const stream = await downloadMediaMessage(
            ...
            { 
                logger,
                // pass this so that baileys3 can request a reupload of media
...
### Reading Messages
- A set of message [keys](https://baileys3.whiskeysockets.io/types/WAMessageKey.html) must be explicitly marked read now.
- You cannot mark an entire 'chat' read as it were with baileys3 Web.
...
- ``` presence ``` can be one of [these](https://baileys3.whiskeysockets.io/types/WAPresence.html)
...
> [!NOTE]
> If a desktop client is active, WA doesn't send push notifications to the device. If you would like to receive said notifications -- mark your baileys3 client offline using `suki.sendPresenceUpdate('unavailable')`
...
> [!NOTE]
> Like media messages, you can pass `{ stream: Stream }` or `{ url: Url }` or `Buffer` directly, you can see more [here](https://baileys3.whiskeysockets.io/types/WAMediaUpload.html)
...
```
- Message body can be a `extendedTextMessage` or `imageMessage` or `videoMessage` or `voiceMessage`, see [here](https://baileys3.whiskeysockets.io/types/AnyRegularMessageContent.html)
- You can add `backgroundColor` and other options in the message options, see [here](https://baileys3.whiskeysockets.io/types/MiscMessageGenerationOptions.html)
...
baileys3 is written with custom functionality in mind. Instead of forking the project & re-writing the internals, you can simply write your own extensions.

### Enabling Debug Level in baileys3 Logs
